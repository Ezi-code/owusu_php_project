...
if (isset($_POST['submit'])) {
  if (empty($_POST['username']) or empty($_POST['email']) or empty($_POST['password']) or empty($_POST['re-password'])) {
    ...
    if ($password == $repassword) {
      ...
      if (strlen($email) > 35  or strlen($username) > 20) {
        echo "<script>alert('E-mail or Username is too long')</script>";
      } else {
        ...
        if ($validate->rowCount() > 0) {
          echo "<script>alert('E-mail or Username is already taken')</script>";
        } else {
